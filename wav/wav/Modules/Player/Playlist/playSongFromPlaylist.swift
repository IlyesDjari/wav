...
extension PlayerViewController {
    internal func playSongsFromPlaylist(_ playlistSong: MusicItemCollection<Track>) {
        Task {
            let songs = playlistSong.compactMap { track -> Song? in
            ...
            let startIndex: Int
            if let selected = playlistSongSelected, selected < songs.count {
                startIndex = selected
            } else {
                startIndex = 0
            }
